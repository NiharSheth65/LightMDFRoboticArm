...
// This fixes the absolute positioning offset
long ELBOW_OFFSET = 0; 
...
void homeRobot() {
    switch(robotHomeState) {
        ...
        case HOME_WRIST:
            ...
            // --- THE OFFSET FIX ---
            // Calculate what the IK math expects at the (0,0,220) home position
            {
                IKResult homeIK = jointAngles(0, 0, 220);
                long homeShoulderSteps = (homeIK.shoulderAngle * SHOULDER_STEPS_PER_REV) / 360.0;
                long homePureElbowSteps = (homeIK.elbowAngle * ELBOW_STEPS_PER_REV) / 360.0;
                
                // Because the physical motor was homed to 0, we find the constant offset
...
stepResult jointSteps(float X, float Y, float Z){
    ...
    // Calculate Absolute Joint Steps from Angles
    ...
    // This perfectly matches the physical behavior of your old code,
    // but translates it into an absolute coordinate so we don't drift during blending!
...
void moveMotors(float xCord, float yCord, float zCord) {
    ...
    ELBOW_STEPPER.moveTo(steps.elbowSteps); 
